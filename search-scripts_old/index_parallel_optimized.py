...
def getUsersData(user_ids, user_range, map_dict=None):
    ...
    last_active_time = FalconDataUtils.getUserLastActiveTime(user_ids=user_ids, map_dict=map_dict,
...
def getProfilesData(user_ids=None, user_range=None, profile_range=None, map_dict=None):
    ...
    profile_preferred_locations, profile_preferred_countries = FalconDataUtils.getProfilePreferredLocationsAndCountries(
    ...
    profile_current_employment, profile_previous_employments = FalconDataUtils.getEmploymentDetails(
        profile_ids=profile_ids, map_dict=map_dict, user_range=user_range, profile_range=profile_range,
        profile_2_site_context=profile_2_site_context)
    ...
    for user_id, profiles in profile_details.items():
        for profile in profiles:
            ...
            profile["preferred_countries"] = profile_preferred_countries.get(profile["profile_id"]) if profile[
                                                                                                           "profile_id"] in profile_preferred_countries else []
            ...
            if resume_parsed_data.get(profile["profile_id"]):
                ...
                if not profile.get("experience") and parsed_data.get("experience") and parsed_data["experience"] > 0 and \
                        parsed_data["experience"] < 100:
                    profile["final_experience"] = parsed_data["experience"]
                    profile["resume_parsed_data"]["experience"] = parsed_data["experience"]
                if ((not profile.get("current_employment")) or (
                not profile["current_employment"].get("designation")) or (
                    not profile["current_employment"]["designation"].get("text"))) and parsed_data.get(
                        "current_designation") and parsed_data["current_designation"].strip() != "-1" and parsed_data[
                    "current_designation"].strip() != "-2":
                    profile["resume_parsed_data"]["current_designation"] = parsed_data["current_designation"]
                if parsed_data.get("skills") and parsed_data["skills"].strip() != "-1" and parsed_data[
                    "skills"].strip() != "-2":
